...
class PostPartTimeJobsController extends GetxController {
  final _formatDate = DateFormat("dd-MM-yyyy");
  final _formatNumber = NumberFormat('#,###');

  DataPost dataPost;

  PostPartTimeJobsController({this.dataPost});
  ...
  Future getImage() async {
    ...
    _imageLogo.value = await UtilsData.compressionFile(File(img.path));
  }
  ...
  void onUpdateJob() {
    if (textEdtCtrlNameJobsRecruitment.text.isEmpty ||
        ...
        texWorkoutTimeCtrl.text.isEmpty) {
      ...
      if(texWorkoutTimeCtrl.text.isEmpty) {
  ...
  void setUp() async {
    ...
    _listFile.value = File(fileName);
    _oldPathFile.value = _listFile.value.path;
